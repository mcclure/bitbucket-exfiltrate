...
def create(username, email, names):
	...
	for line in names:
		...
		payload = RepositoryPayload({'is_private':False, 'fork_policy': RepositoryForkPolicy.ALLOW_FORKS, 'description': "This repository has moved. See wiki for new link.", 'has_issues':False, 'has_wiki':True})
		try:
			Repository.create(payload, name, None, bitbucket) # Returns repo but it isn't used
